...
input_bls<-args[2]
input_tree_order <- args[3]
input_past_trans_probs <- args[4]
input_past_gen_clusts <- args[5]
input_prob_threshold <- as.numeric(args[6])
input_new_cluster <- as.logical(args[7])
...
TransProbs<-function(dataInput,contactData,probThreshold,dateThreshold,restrictClusters,beta){
  ...
  colnames(transmat)<-c("host1","host2",paste0("Prob_",min(probThreshold)))
...
cov2clusters<-function(treeName=input_tree,metafile=NA, contactData=FALSE,contactFile=NA,
                       json_dates=TRUE,json_file=input_bls,
  ...
  dataInput$dates<-round(lubridate::decimal_date(as.Date(dates$date))*365)
  ...
  for (threshold in 1:length(probThreshold)){
    transmat<-TransProbs(dataInput,contactData,probThreshold[threshold],dateThreshold,restrictClusters,beta)
    if (!newClustering){
...
cov2clusters(treeName = input_tree_order,returnTransProbs=TRUE,pastTransProbs=input_past_trans_probs,
             probThreshold = input_prob_threshold,newClustering = input_new_cluster,
